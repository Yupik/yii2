...
Yii dostarcza zestaw powszechnie używanych podstawowych walidatorów, znajdujących się przeważnie pod przestrzenią nazw `yii\validators`.
Zamiast używania długich nazw klas walidatorów, możesz użyć ich *aliasów* aby określić, który z nich powinien zostać użyty.
Dla przykładu, możesz użyć aliasu `required` aby odnieść się do klasy [[yii\validators\RequiredValidator]]:
...
Własciwość [[yii\validators\Validator::builtInValidators]] deklaruje wszystkie wspierane aliasy walidatorów.

W dalszej części opiszemy użycie i właściwości wszystkich podstawowych walidatorów.
...
```php
[
    // sprawdza czy atrybut "selected" jest równy 1 lub 0, niezależnie od typu danych
    ['selected', 'boolean'],

    // sprawdza czy atrybut "deleted" jest typem danych `boolean`, zarówno `true` jak i `false`
    ['deleted', 'boolean', 'trueValue' => true, 'falseValue' => false, 'strict' => true],
]
```

Ten walidator sprawdza, czy dana wartość jest wartością logiczną.

- `trueValue`: wartość reprezentująca *true*. Domyślnie `'1'`.
- `falseValue`: wartość reprezentująca *false*. Domyślnie `'0'`.
- `strict`: określa, czy typ danych wejściowych powinien być taki sam, jak wartość `trueValue` oraz `falseValue`. Domyślnie ustawione na `false`.


> Note: Wszystkie dane wysłane przez formularze HTML są typu `string`, dlatego zazwyczaj powinieneś zostawić wartość [[yii\validators\BooleanValidator::strict|strict]] ustawioną na false.
